...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		$data = json_decode('{"key":"","expire_time":0}');
        $data->key = $value;
        $data->expire_time = $expired + time();
        $success = Service::factory('VariableService')->setVariable($cachename, json_encode($data));
		return $success;
	}
	...
	protected function getCache($cachename){
		$data = json_decode(Service::factory('VariableService')->getVariable($cachename));
