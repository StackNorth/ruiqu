...
class XingeApp {
	...
	public static function PushTokenAndroid($accessId, $secretKey, $title, $content, $token)
	{
		...
		$mess = new PushMessage();
		$mess->setTitle($title);
		$mess->setContent($content);
		$mess->setType(PushMessage::TYPE_NOTIFICATION);
	...
	public function  PushSingleDevice($deviceToken, $message, $environment=0)
	{
		...
		if (!($message instanceof PushMessage) && !($message instanceof MessageIOS)) return $ret;
		...
		if($message instanceof PushMessage) $params['multi_pkg'] = $message->getMultiPkg();
	...
	public function  PushSingleAccount($deviceType, $account, $message, $environment=0)
	{
		...
		if (!($message instanceof PushMessage) && !($message instanceof MessageIOS))
		{
	...
	private function ValidateMessageType($message)
	{
		if(($this->accessId) >= XingeApp::IOS_MIN_ID and $message instanceof MessageIOS)
			return true;
		else if(($this->accessId) < XingeApp::IOS_MIN_ID and $message instanceof PushMessage)
			return true;
		else
			return false;
	}
