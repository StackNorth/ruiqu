	...
	F.helpers.buttons = {
		...
		afterShow: function (opts, obj) {
			...
			if (!buttons) {
				...
				var ml = parseInt((w_width - 250) / 2);
				$('#fancybox-buttons').css('left', ml + 'px');
				buttons = {
					prev   : this.list.find('.btnPrev').click( F.prev ),
					next   : this.list.find('.btnNext').click( F.next ),
					play   : this.list.find('.btnPlay').click( F.play ),
					toggle : this.list.find('.btnToggle').click( F.toggle ),
					left : this.list.find('.btnLeftRotate').click(function(){
						angle -= 90;
						$('.fancybox-image').css({
							'-webkit-transform' : 'rotate(' + angle + 'deg)'
						})
					}),
					right : this.list.find('.btnRightRotate').click(function(){
						angle += 90;
						$('.fancybox-image').css({
							'-webkit-transform' : 'rotate(' + angle + 'deg)'
						})
